				     ...
				     struct job_irq_info *info, int status)
{
	...
	if (cluster->partition_mode == PARTITION_MODE_NONE ||
		...
		if (IS_CLUSTER_IRQ_V4(status) && IS_FAULT_IRQ_V4(status)) {
			...
			cluster->event_type = AIPU_IRQ_EVENT_RESET;
		} else if (IS_TEC_IRQ_V4(status) && (IS_FAULT_IRQ_V4(status))) {
...
static int zhouyi_v4_upper_half(void *data)
{
	...
	for (id = 0; id < TSM_IRQ_MAX_NUM; id++) {
		...
		if (IS_ERROR_IRQ_V4(status) &&
			...
			dev_err(cluster->dev, "TCB format error or bus error in PCP or SCP");
...
bool zhouyi_v4_enable_debug_mode(struct aipu_partition *cluster)
{
	int value = 0;

	value = aipu_read32(cluster->reg, AHB_INTERNAL_CSR_SELECTION_CTRL_REG);
	value |= BIT(12);
	aipu_write32(cluster->reg, AHB_INTERNAL_CSR_SELECTION_CTRL_REG, value);

	value = aipu_read32(cluster->reg, PMU_TOP_CLOCK_POWER_CTRL_REG);
	value &= ~BIT(2);
	aipu_write32(cluster->reg, PMU_TOP_CLOCK_POWER_CTRL_REG, value);

	value = aipu_read32(cluster->reg, COMMAND_POOL_PCP_SECURE_CONFIG_REG);
	value &= ~BIT(0);
	aipu_write32(cluster->reg, COMMAND_POOL_PCP_SECURE_CONFIG_REG, value);

	value = aipu_read32(cluster->reg, COMMAND_POOL_SCP_SECURE_CONFIG_REG);
	value &= ~BIT(0);
	aipu_write32(cluster->reg, COMMAND_POOL_SCP_SECURE_CONFIG_REG, value);

	return true;
}

bool zhouyi_v4_disable_debug_mode(struct aipu_partition *cluster)
{
	int value = 0;
...
static struct aipu_operations zhouyi_v4_ops = {
	...
	.enable_debug_mode = zhouyi_v4_enable_debug_mode,
	.disable_debug_mode = zhouyi_v4_disable_debug_mode,
